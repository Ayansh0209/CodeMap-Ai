...
exports.buildRetrievalIndex = buildRetrievalIndex;
function detectSemanticRole(fileId, isBarrel) {
    if (isBarrel)
        return "barrel";
 ...
 */
function buildImportedByMap(importEdges) {
    const map = new Map();
 ...
 */
function buildImportsMap(importEdges) {
    const map = new Map();
    for (const edge of importEdges) {
 ...
 */
function resolveBarrelTargets(fileId, importEdges) {
    return importEdges
 ...
 */
function buildRetrievalIndex(owner, repo, commitSha, fileNodes, importEdges, allFunctions) {
    ...
    const filesWithReExports = new Set(importEdges
    ...
    const files = fileNodes.map((file) => {
        ...
        const isBarrel = file.isBarrel === true || (fileFunctionsRaw.length === 0 &&
    ...
    const authCount = files.reduce((sum, f) => sum + f.functions.filter(fn => fn.hasAuthCheck).length, 0);
    const dbCount = files.reduce((sum, f) => sum + f.functions.filter(fn => fn.hasDatabaseCall).length, 0);
    const barrelCount = files.filter(f => f.isBarrel).length;
    console.log(`[retrievalBuilder] built retrieval index: ` +
        `${files.length} files, ` +
        `${barrelCount} barrels, ` +
