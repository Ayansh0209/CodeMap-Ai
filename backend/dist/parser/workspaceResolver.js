...
exports.detectWorkspaces = detectWorkspaces;
exports.resolveFilePackage = resolveFilePackage;
...
function expandWorkspaceGlob(repoRoot, pattern) {
    ...
    const parentDir = path_1.default.resolve(repoRoot, cleaned);
    if (!fs_1.default.existsSync(parentDir))
        return [];
    try {
        const stat = fs_1.default.statSync(parentDir);
    ...
    try {
        ...
        for (const entry of entries) {
            if (entry.startsWith(".") || entry === "node_modules")
                continue;
            const full = path_1.default.join(parentDir, entry);
            try {
 ...
 */
function readPackageInfo(pkgDir, repoRoot, allPkgNames) {
    ...
    const root = path_1.default.relative(repoRoot, pkgDir).replace(/\\/g, "/") || ".";
...
function detectWorkspaces(repoRoot) {
    ...
    let tool = "none";
    // ── 1. pnpm-workspace.yaml ────────────────────────────────────────────────
    const pnpmYaml = safeReadText(path_1.default.join(repoRoot, "pnpm-workspace.yaml"));
    if (pnpmYaml) {
        tool = "pnpm";
    ...
    if (workspaceDirs.length === 0) {
        const turboJson = safeReadJson(path_1.default.join(repoRoot, "turbo.json"));
        if (turboJson) {
            tool = "turbo";
    ...
    // ── 3. nx.json ────────────────────────────────────────────────────────────
    if (workspaceDirs.length === 0) {
        const nxJson = safeReadJson(path_1.default.join(repoRoot, "nx.json"));
        if (nxJson) {
            tool = "nx";
    ...
    // ── 4. lerna.json ─────────────────────────────────────────────────────────
    if (workspaceDirs.length === 0) {
        const lernaJson = safeReadJson(path_1.default.join(repoRoot, "lerna.json"));
        if (lernaJson) {
            tool = "lerna";
    ...
    if (workspaceDirs.length === 0) {
        ...
        if (rootPkg) {
            ...
            if (patterns.length > 0) {
                tool = tool === "none"
                    ? (fs_1.default.existsSync(path_1.default.join(repoRoot, "yarn.lock")) ? "yarn" : "npm")
    ...
    if (workspaceDirs.length === 0) {
        for (const dir of ["packages", "apps", "libs", "modules", "services"]) {
            const full = path_1.default.join(repoRoot, dir);
    ...
    for (const dir of uniqueDirs) {
        const pkg = safeReadJson(path_1.default.join(dir, "package.json"));
    ...
    if (packages.length > 0) {
        console.log(`[workspaceResolver] detected ${packages.length} workspace packages ` +
            `(tool: ${tool}, monorepo: ${isMonorepo})`);
        for (const pkg of packages.slice(0, 10)) {
            console.log(`  ${pkg.name} @ ${pkg.root} ` +
 ...
 */
function resolveFilePackage(filePath, packages) {
