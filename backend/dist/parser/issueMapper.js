...
exports.MIN_CANDIDATES_FOR_STAGE1 = void 0;
exports.traverseGraph = traverseGraph;
exports.buildCompactGraphMap = buildCompactGraphMap;
exports.mapIssueToCode = mapIssueToCode;
exports.buildInlineSearchIndex = buildInlineSearchIndex;
 ...
 */
exports.MIN_CANDIDATES_FOR_STAGE1 = 5;
// ── Noise filter ──────────────────────────────────────────────────────────────
/**
 ...
 */
function isNoisePath(fileId) {
    const lower = fileId.toLowerCase();
    // TypeScript declaration files — zero behavior
    if (lower.endsWith(".d.ts"))
        return true;
    // Auto-generated documentation
    if (lower.includes("/auto-docs/") || lower.includes("/auto-schema/"))
        return true;
    // Test files — only allowed via PR, not via token match
    if (/\.(test|spec)\.(ts|js|tsx|jsx)$/i.test(lower))
        return true;
    if (/\/__tests?__\//i.test(lower))
        return true;
 ...
 */
function expandBarrels(fileIds, fileMap) {
 ...
 */
function addNeighborhood(directCandidates, fileMap, maxToAdd) {
    const result = new Set(directCandidates);
    let added = 0;
 ...
 */
function traverseRetrievalGraph(intent, retrieval, linkedPRs, graphFileIds) {
    const fileMap = new Map();
    for (const f of retrieval.files) {
        fileMap.set(f.fileId, f);
    }
    // ── PR-based files (always included) ──────────────────────────────────────
    const prFileIds = new Set();
    ...
    for (const fileEntry of retrieval.files) {
        // Skip noise paths
        if (isNoisePath(fileEntry.fileId))
            continue;
    ...
    // Cap direct matches
    const cappedMatches = new Set([...matchedFileIds].slice(0, MAX_DIRECT_CANDIDATES));
    // ── Barrel expansion ──────────────────────────────────────────────────────
 ...
 */
function traverseGraph(intent, retrieval, linkedPRs, graphFileIds) {
 ...
 */
function buildCompactGraphMap(retrieval) {
 ...
 */
function mapIssueToCode(query, index, maxResults = 10) {
    const tokens = extractQueryTokens(query);
    if (tokens.length === 0) {
        return { issueText: query, matchedKeywords: [], topFiles: [], topFunctions: [], confidenceScore: 0 };
    }
    const fileResults = (0, queryEngine_1.searchIndex)(index, tokens.join(" "), { type: "file", limit: maxResults * 2, scoreThreshold: 20 });
    const exportResults = (0, queryEngine_1.searchIndex)(index, tokens.join(" "), { type: "export", limit: maxResults * 2, scoreThreshold: 20 });
    const testResults = (0, queryEngine_1.searchIndex)(index, tokens.join(" "), { type: "test", limit: maxResults, scoreThreshold: 20 });
    ...
    const candidateFunctionMap = new Map();
    ...
    if (topFiles.length > 0)
        confidenceScore = topFiles[0].score;
 ...
 * @deprecated Use traverseGraph() instead
 */
function buildInlineSearchIndex(files) {
    const entries = files.map(f => {
        ...
        return { id: f.id, type: "file", name: f.label, filePath: f.id, tokens, hubScore: f.architecturalImportance ?? 0 };
    });
