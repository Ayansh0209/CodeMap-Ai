...
function isInPenaltyFolder(filePath) {
 ...
 */
function isInUtilityFolder(filePath) {
    const segments = filePath.split("/");
 ...
 */
function isFrameworkSemanticFile(file) {
 ...
 */
function findUnusedExports(fileId, functions, allImportEdges) {
 ...
 */
function findOrphanSymbols(fileId, functions) {
 ...
 */
function analyzeDeadCode(fileNodes, importEdges, allFunctions) {
    // Build degree maps
    ...
    console.log(`[deadCodeAnalyzer] scored ${fileNodes.length} files — ` +
