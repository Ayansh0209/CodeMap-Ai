 ...
 */
function selectFunctions(functions, source, tokens) {
 ...
 */
function semanticTruncate(body) {
    const lines = body.split("\n");
    if (lines.length <= HUGE_FUNCTION_LINES)
        return body;
    const HEAD_LINES = 10;
    const TAIL_LINES = 10;
    ...
    for (let i = 0; i < middle.length; i++) {
        if (isHighSignal(middle[i])) {
            for (let j = Math.max(0, i - CONTEXT_AROUND_HIGH_SIGNAL); j <= Math.min(middle.length - 1, i + CONTEXT_AROUND_HIGH_SIGNAL); j++) {
                keepIndices.add(j);
            }
        }
    }
 ...
 */
async function fetchRawFileCached(owner, repo, commitSha, fileId) {
    ...
    const content = await (0, issueClient_1.fetchRawFile)(owner, repo, commitSha, fileId);
    if (content) {
        try {
            await jobQueue_1.redisConnection.set(cacheKey, content, "EX", RAW_FILE_CACHE_TTL_SECONDS);
        }
        catch {
 ...
 */
function sliceFunctionBody(rawContent, startLine, endLine) {
 ...
 */
async function fetchSnippets(candidates, retrieval, intent, owner, repo, commitSha) {
    ...
    for (const { candidateEntry, selectedFunctions } of selectedFiles) {
        if (snippets.length >= MAX_TOTAL_SNIPPETS)
            break;
        ...
        if (rawContent === undefined) {
            try {
                ...
                console.warn(`\x1b[31m[snippetFetcher] failed to fetch ${fileId}:\x1b[0m`, err.message);
                continue;
            }
        }
        if (!rawContent)
    ...
    console.log(`\x1b[32m[snippetFetcher] selected ${snippets.length} snippets from ` +
