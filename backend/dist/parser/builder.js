...
const entryScorer_1 = require("./entryScorer");
const graphAnalytics_1 = require("./graphAnalytics");
const workspaceResolver_1 = require("./workspaceResolver");
const deadCodeAnalyzer_1 = require("./deadCodeAnalyzer");
const searchIndexer_1 = require("../search/searchIndexer");
const repoMetadata_1 = require("./repoMetadata");
// ── Helpers ───────────────────────────────────────────────────────────────────
function sanitizeFileId(relativePath) {
...
function deduplicateImportEdges(edges) {
    const seen = new Set();
    const result = [];
...
function buildGraph(input) {
    const { owner, repo, commitSha, fileNodes, importEdges, allFunctions, startupSignals, routeHandlers, repoRoot } = input;
    ...
    (0, entryScorer_1.applyEntryScoring)(fileNodes, validImportEdges, {
        repoRoot: repoRoot ?? "",
    ...
    const analyticsStats = (0, graphAnalytics_1.applyGraphAnalytics)(fileNodes, validImportEdges);
    ...
    for (const file of fileNodes) {
        ...
        if (architecturalImportance > 100)
            architecturalImportance = 100;
    ...
    top5.forEach(f => console.log(`  ${f.id} — importance: ${f.architecturalImportance} hub: ${f.hubScore} entry: ${f.isEntryPoint}`));
    // ── Step 2.9: Workspace / Monorepo Resolution ────────────────────────────
    const workspaceInfo = repoRoot ? (0, workspaceResolver_1.detectWorkspaces)(repoRoot) : undefined;
    if (workspaceInfo && workspaceInfo.packages.length > 0) {
        for (const file of fileNodes) {
            const pkg = (0, workspaceResolver_1.resolveFilePackage)(file.id, workspaceInfo.packages);
            if (pkg) {
    ...
    const deadCodeStats = (0, deadCodeAnalyzer_1.analyzeDeadCode)(fileNodes, validImportEdges, allFunctions);
    ...
    const callEdges = [];
    const calledByMap = new Map(); // targetId → Set of callerIds
    for (const fn of allFunctions) {
        for (const rawCallName of fn.calls) {
            ...
            const sameFile = candidates.find((id) => id.startsWith(fn.filePath + "::"));
    ...
    if (repoRoot) {
        graphData.repoMetadata = (0, repoMetadata_1.extractRepoMetadata)(repoRoot, workspaceInfo?.packages ?? []);
    }
    ...
    for (const file of fileNodes) {
        ...
        const fileCallEdges = uniqueCallEdges.filter((e) => fileFunctionIds.has(e.source) || fileFunctionIds.has(e.target));
    ...
    const searchIndex = (0, searchIndexer_1.buildSearchIndex)(fileNodes, allFunctions, validImportEdges);
