...
const fileLevel_1 = require("./fileLevel");
const functionLevel_1 = require("./functionLevel");
...
function detectFileKind(relativePath, sourceFile) {
...
function detectIsEntryPoint(relativePath) {
    const filename = path_1.default.basename(relativePath).toLowerCase();
...
async function processChunk(decisions, repoRoot, resolver, chunkIndex) {
    const fileNodes = [];
    const importEdges = [];
    const allFunctions = [];
    const startupSignals = new Map();
    ...
    for (const decision of decisions) {
        try {
            ...
            console.warn(`[chunkProcessor] Could not add file: ${decision.relativePath} — ${err.message}`);
        }
    }
    // Process each file
    for (const decision of decisions) {
        ...
        try {
            ...
            const fileLevelResult = (0, fileLevel_1.extractFileLevel)(sourceFile, decision.relativePath);
            ...
            for (const rawImport of fileLevelResult.rawImports) {
                const resolved = resolver.resolve(rawImport.specifier, decision.absolutePath);
            ...
            const extracted = decision.mode === "full"
                ? (0, functionLevel_1.extractFunctionLevel)(sourceFile, decision.relativePath)
                : { functions: [], structures: [] };
            const functions = extracted.functions;
            const structures = extracted.structures;
            // Debug: warn if a JS file in full-parse mode produced 0 functions
            if (decision.mode === "full" && functions.length === 0) {
                const ext = path_1.default.extname(decision.relativePath);
                if (ext === ".js" || ext === ".jsx") {
            ...
            let testCases = [];
            if (kind === "test") {
                const metadata = (0, functionLevel_1.extractTestMetadata)(sourceFile);
                testSuites = metadata.testSuites;
                testCases = metadata.testCases;
            }
            fileNodes.push({
                id: decision.relativePath,
                label: path_1.default.basename(decision.relativePath),
                language,
            ...
            console.warn(`[chunkProcessor] Parse error in ${decision.relativePath}: ${err.message}`);
            fileNodes.push({
                id: decision.relativePath,
                label: path_1.default.basename(decision.relativePath),
                language: "unknown",
...
// ── Main entry point ──────────────────────────────────────────────────────────
async function processAllFiles(decisions, repoRoot, onProgress) {
    ...
    const resolver = new importResolver_1.ImportResolver(repoRoot);
    const chunks = chunkArray(filesToParse, CHUNK_SIZE);
    const allFileNodes = [];
    const allImportEdges = [];
    const allFunctions = [];
    const allStartupSignals = new Map();
    const allRouteHandlers = new Map();
    console.log(`[chunkProcessor] ${filesToParse.length} files to parse in ${chunks.length} chunks`);
    let processedCount = 0;
    for (let i = 0; i < chunks.length; i++) {
        const chunk = chunks[i];
        console.log(`[chunkProcessor] chunk ${i + 1}/${chunks.length} — ${chunk.length} files`);
    ...
    console.log(`[chunkProcessor] done — ` +
