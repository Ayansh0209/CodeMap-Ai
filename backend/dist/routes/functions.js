 ...
 */
function functionsCacheKey(owner, repo, commitSha, fileId) {
...
async function loadRetrievalIndex(owner, repo) {
    try {
        const raw = await jobQueue_1.redisConnection.get(`retrieval:${owner}:${repo}`);
        if (!raw)
            return null;
 ...
 */
async function fetchMergedFunctions(targets, owner, repo, commitSha) {
    const merged = [];
...
router.post("/", async (req, res) => {
    try {
        ...
        const directCached = await jobQueue_1.redisConnection.get(directKey);
        if (directCached) {
        ...
        console.log(`[functions] barrel detected: ${fileId} → ` +
            `[${fileEntry.barrelTargets.slice(0, 3).join(", ")}` +
            `${fileEntry.barrelTargets.length > 3 ? `, +${fileEntry.barrelTargets.length - 3} more` : ""}]`);
        const merged = await fetchMergedFunctions(fileEntry.barrelTargets, owner, repo, commitSha);
        if (merged.length === 0) {
