...
// ── Runner ───────────────────────────────────────────────────────────────────
function runTest(testName, query, expectedTopFile) {
    console.log(`\n▶ TEST: ${testName}`);
    console.log(`  Query: "${query}"`);
    const result = (0, issueMapper_1.mapIssueToCode)(query, mockIndex, 5);
...
console.log("========================================");
runTest("Auth bug targeting login function", "Users cannot login, the auth service is throwing an error", "src/auth/authService.ts");
runTest("Reducer state bug", "Fix user state bug in the userReducer", "src/store/userReducer.ts");
runTest("Route API issue", "The /api/routes/loginRoute is returning 500", "src/api/routes/loginRoute.ts");
runTest("Middleware bug", "Auth middleware is blocking requests", "src/middleware/authMiddleware.ts");
