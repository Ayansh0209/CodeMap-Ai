 ...
 */
function buildSearchIndex(fileNodes, allFunctions, importEdges) {
    const entries = [];
    ...
    for (const file of fileNodes) {
        ...
        if (file.isEntryPoint)
            tokens.push("entry", "entrypoint");
        if (file.isDeadCode)
            tokens.push("dead", "deadcode", "unused");
        ...
        if (lowerId.includes("route"))
            tokens.push("route");
        if (lowerId.includes("middleware"))
            tokens.push("middleware");
        if (lowerId.includes("service"))
            tokens.push("service");
        if (lowerId.includes("controller"))
            tokens.push("controller");
        if (lowerId.includes("auth"))
            tokens.push("auth");
        if (lowerId.includes("hook") || lowerId.includes("use"))
            tokens.push("hook");
        if (lowerId.includes("store") || lowerId.includes("slice") || lowerId.includes("reducer"))
            tokens.push("state", "reducer", "store");
        entries.push({
            id: file.id,
    ...
    for (const fn of allFunctions) {
        if (!fn.isExported)
            continue; // only index exports for search
        const tokens = [
            ...tokenize(fn.name),
    ...
    console.log(`[searchIndexer] built index with ${entries.length} entries ` +
